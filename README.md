...
## 🚀 Getting Started
### 1. Local Development
To run the ETL pipeline and validate the code locally:
```bash
...
### 2. CI/CD Workflow
The CI pipeline is defined in `.github/workflows/ci.yml`. It was generated by **Llama 3.1 8b** and includes:
- **Linting**: Automated PEP 8 compliance checks via `flake8`.
- **Testing**: Regression testing via `pytest`.
- **Coverage**: Visual reporting of code coverage percentages.

## 📈 Engineering Decisions
- **Caching Strategy**: The CI uses GitHub's caching mechanism to reduce build times by up to 60%.
- **Healthcare Logic**: The ETL includes claim status normalization and tax calculation, simulating real-world healthcare business rules.
